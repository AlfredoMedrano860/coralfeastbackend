    ...
    public function up(): void
    {
        ...
        DB::table('inventory_item_categories')
            ...
            ->each(function ($category): void {
                DB::table('inventory_item_categories')
                    ->where('id', $category->id)
                    ->update(['slug' => 'category-'.$category->id]);
            });

        $driver = DB::getDriverName();

        if ($driver === 'pgsql') {
            DB::statement('ALTER TABLE inventory_item_categories ALTER COLUMN slug SET NOT NULL');
            DB::statement('ALTER TABLE inventory_item_categories ADD CONSTRAINT inventory_item_categories_slug_unique UNIQUE (slug)');
        } else {
