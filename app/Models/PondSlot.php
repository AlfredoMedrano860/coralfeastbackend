...
class PondSlot extends Model
{
    ...
    protected static function booted(): void
    {
        static::saving(function (self $slot): void {
            $slot->stage_progress_seconds = (int) round((float) ($slot->stage_progress_seconds ?? 0));
            $slot->stage_duration_seconds = (int) round((float) ($slot->stage_duration_seconds ?? 0));
        });
    }

    public function __set($key, $value)
    {
        if ($key === 'stage_progress_seconds' || $key === 'stage_duration_seconds') {
    ...
    {
        $attributes = $this->sanitizeProgressAttributes($attributes);
    ...
    protected function sanitizeProgressAttributes(array $attributes): array
    {
        if (array_key_exists('stage_progress_seconds', $attributes)) {
            $attributes['stage_progress_seconds'] = (int) round((float) ($attributes['stage_progress_seconds'] ?? 0));
        }

        if (array_key_exists('stage_duration_seconds', $attributes)) {
            $attributes['stage_duration_seconds'] = (int) round((float) ($attributes['stage_duration_seconds'] ?? 0));
        }
